<?php
    ...
    class IncomeExpenseCategory extends Controller {
        ...
        public function store(Request $request) {
            ...
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.income-expense-category.edit", $incomeExpenseCategory->id);
        }
        ...
        public function update(Request $request, $id) {
            ...
            if (!$incomeExpenseCategory) {
                ...
                return redirect()->route("panel.income-expense-category.index");
            }
            $incomeExpenseCategory->fill($data);
            $incomeExpenseCategory->save();
            session()->flash('success', trans("app.İşlem Başarılı"));
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.income-expense-category.edit", $id);
        }
        public function destroy($id) {
            ...
            return request()->has("redirect") ? redirect()->to(request()->get("redirect"))->withInput() : redirect()->route("panel.income-expense-category.index")->withInput();
        }
    }
