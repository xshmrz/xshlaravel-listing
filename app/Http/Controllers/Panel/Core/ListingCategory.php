<?php
    ...
    class ListingCategory extends Controller {
        ...
        public function store(Request $request) {
            ...
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.listing-category.edit", $listingCategory->id);
        }
        ...
        public function update(Request $request, $id) {
            ...
            if (!$listingCategory) {
                ...
                return redirect()->route("panel.listing-category.index");
            }
            $listingCategory->fill($data);
            $listingCategory->save();
            session()->flash('success', trans("app.İşlem Başarılı"));
            return $request->has("redirect") ? redirect()->to($request->get("redirect")) : redirect()->route("panel.listing-category.edit", $id);
        }
        public function destroy($id) {
            ...
            return request()->has("redirect") ? redirect()->to(request()->get("redirect"))->withInput() : redirect()->route("panel.listing-category.index")->withInput();
        }
    }
